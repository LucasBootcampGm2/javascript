...
let movie = {
  ...
  Age: 2000,
  Characters: [
    { name: "Deadpool", species: "Mutant", rol: "" },
    { name: "Wolverine", species: "Mutant", rol: "Main" },
  ],
};
console.log(movie);
let { Title } = movie;
...
let eventObject = {
  ...
  Date: "12/3",
  Guests: [
    { name: "jiji", species: "sdsd", confirm: true },
    { name: "jeje", species: "sdsd", confirm: true },
  ],
};
...
let charactersArray = [
  { Name: "jose lui", Kingdom: "Swamp" },
  { Name: "Martita", Kingdom: "Not a Swamp" },
];
...
// Crear un objeto llamado jugadores, que contiene las propiedades nombre (Lista de nombres) y numero de camiseta (Lista de numeros de camiseta) del jugador, y crea una funcion que devuelva un nuevo objeto con una nueva lista de los nombres y una nueva lista de los numeros de los jugadores con un numero de camiseta mayor a 9.

let players = {
  Name: ["Hi", "Lucas", "welcome", "to", "Gm2"],
  TshirtNumber: [12, 11, 2, 3, 4],
};

function getName(playersObject) {
  let newObject = {
    Name: [],
    TshirtNumber: [],
  };
  let array = Object.values(playersObject.TshirtNumber);
  array.filter(function (num, index) {
    if (num > 9) {
      newObject.Name.push(playersObject.Name[index]);
      newObject.TshirtNumber.push(playersObject.TshirtNumber[index]);
    }
  });
  return newObject;
}
